...
async function main() {
  ...
  const migration = readFileSync("drizzle/0014_subscriptions_refactor.sql", "utf-8");
  ...
  for (const c of cols) {
    console.log(`  ${c.column_name.padEnd(26)} ${c.data_type.padEnd(28)} ${c.is_nullable === "NO" ? "NOT NULL" : ""}`);
  }
  ...
  const eventCols = await sql`
    ...
    ORDER BY ordinal_position
  `;
  console.log("\nFinal processed_stripe_events columns:");
  for (const c of eventCols) {
    console.log(`  ${c.column_name.padEnd(26)} ${c.data_type}`);
  }
