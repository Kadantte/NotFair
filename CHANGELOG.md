...
### Fixed
- **GoHighLevel OAuth requests the documented document-contract templates scope.** Replaces the singular `documents_contracts_template/list.readonly` scope with HighLevel's documented `documents_contracts_templates/list.readonly` scope so Marketplace installs do not fail before the NotFair callback.
...
### Fixed
- **Google Ads MCP now prevents auto-generated extension assets from being linked.** Existing callout, structured snippet, and sitelink asset listings expose `asset.source` plus an `autoGenerated` flag, and link tools now reject non-advertiser-provided assets before calling Google so agents avoid `asset_link_error=20`.
