...
- Canonical audit pack: ads.queries.auditPack(start,end) -> 20 named queries (the gaqlParallel per-call limit) covering account info, campaigns, keywords, search terms, ads/assets, negatives, conversion actions/performance, recommendations, network segmentation, landing pages, paused campaigns, and recent change events. Prefer this for account audits instead of hand-selecting a narrow subset. For billing setup, shared negative list members, or manager links, query ads.queries.billingSetups / sharedNegativeKeywordMembers / customerManagerLinks separately after the main audit.
