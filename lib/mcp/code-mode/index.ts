...
- **Known hallucinated fields:** there is no \`metrics.average_cpc_micros\`, \`metrics.cost_per_conversion_micros\`, \`metrics.impression_share\`, \`metrics.conversion_rate\`, \`metrics.quality_info.quality_score\`, \`asset.status\`, \`asset_group_asset.performance_label\`, \`asset.sitelink_asset.final_urls\`, \`campaign.url_expansion_opt_out\`, \`campaign.budget_micros\`, \`campaign.budget_amount_micros\`, \`campaign_criterion.proximity.address.city\`, \`campaign_criterion.audience.audience\`, \`ad_group_criterion.quality_info.ad_relevance\`, \`ad_group_criterion.quality_info.landing_page_experience\`, \`campaign_experiment.*\`, \`conversion_action.default_value\`, \`conversion_action.last_conversion_date\`, \`conversion_action.most_recent_conversion_date\`, \`recommendation.impact.base_metrics.*\`, \`recommendation.keyword_match_type\`, \`auction_insight.domain\`, or bare \`resource_name\`. Use \`metrics.average_cpc\`; use \`metrics.cost_per_conversion\`; for Search campaigns use \`metrics.search_impression_share\`; calculate conversion rate from \`metrics.conversions / metrics.clicks\`; budget lives on \`campaign_budget.amount_micros\`; asset serving status lives on the link resource (\`campaign_asset.status\`, \`ad_group_asset.status\`, \`asset_group_asset.status\`, \`customer_asset.status\`); use \`campaign_criterion.proximity.address.city_name\`; use \`conversion_action.value_settings.default_value\`; replace \`resource_name\` with \`<resource>.resource_name\`; call \`getResourceMetadata(<resource>)\` for the rest.
