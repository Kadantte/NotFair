...
export const registerReadTools: ToolRegistrar = (server, currentAuth) => {
  ...
  server.registerTool("summarizeAccountSetup", {
    description:
      "One-shot, human-readable snapshot of how the account is configured: currency + time zone, every non-removed campaign with its bidding strategy and tCPA/tROAS in major units, every conversion action with category + primary_for_goal flag, plus diagnostic notes when the setup is unusual (no primary conversion action, mixed optimization modes). Preflight: if the user has not given an explicit accountId (e.g. they referenced a campaign by name without specifying which account), call listConnectedAccounts first — it returns account names so you can identify the right accountId before calling this tool, avoiding an immediate failure on a disconnected ID. Call this FIRST in any strategic conversation — it gives you the conversion hierarchy and bidding posture as named strings so you don't misread enum integers (the BiddingStrategyType landmines: 10=MAXIMIZE_CONVERSIONS, 11=MAXIMIZE_CONVERSION_VALUE, 9=TARGET_SPEND, 15=TARGET_IMPRESSION_SHARE) or treat micros as dollars. Replaces 3+ runScript calls (account info + campaigns + conversion actions) for the canonical setup question.",
    inputSchema: {
      accountId: accountIdParam,
    },
    annotations: READ_ANNOTATIONS,
