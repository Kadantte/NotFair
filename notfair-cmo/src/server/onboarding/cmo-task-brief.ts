...
export function buildOnboardingBrief(args: {
  ...
  const brief = [
    ...
    "`list_project_agents` to get exact agent ids). Phrase each delegated",
    "brief as \"SET UP a recurring <cadence> ...\" with the cadence explicit",
    "(\"set up a daily 9am Pacific anomaly check on enabled campaigns\",",
    "\"set up a weekly Monday search-term review\") — the specialist",
    "schedules its own cron, and the cron IS the deliverable; a brief that",
    "reads like a one-off gets run once and never again.",
    "",
