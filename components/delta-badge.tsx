...
export function DeltaBadge({ before, after, size = 14 }: { before: string; after: string; size?: number }) {
  ...
  if (before === after) return <Minus className={cls} style={style} color="#C4C0B666" />;
  ...
  if (!before || !after) return <ChevronRight className={cls} style={style} color="#C4C0B666" />;
  ...
  return <ChevronRight className={cls} style={style} color="#C4C0B666" />;
}
