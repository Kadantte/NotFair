...
- **Some segments aren't selectable on view resources (`query_error=51`).** Specifically: `segments.hour` cannot be SELECTed on `keyword_view`, `search_term_view`, or `user_location_view` — query `FROM campaign` (or `ad_group`) with `segments.hour` instead. `segments.geo_target_country` / `segments.geo_target_state` cannot be SELECTed on `user_location_view` or `geographic_view`; use `user_location_view.country_criterion_id` / `geographic_view.country_criterion_id` and resolve names via `geo_target_constant`. Bare `conversion_action` is never a selectable SELECT field — use `segments.conversion_action` / `segments.conversion_action_name` from a campaign/ad_group FROM, or query `FROM conversion_action` with `conversion_action.*` fields. Verify at `validateSegmentResourceCompatibility` in `lib/google-ads/reads/gaql.ts`.
...
- **`customer_manager_link.status` has no `REMOVED`.** Valid manager-link status names are `ACTIVE`, `INACTIVE`, `PENDING`, `REFUSED`, and `CANCELED`; omit the status filter if you need all rows.
- **GAQL has no SQL `JOIN`.** Select compatible related-resource fields directly from one `FROM` resource, or run two queries and join rows in JavaScript.
...
- **`mutateResources` generates field masks via `recursiveFieldMaskSearch`** in [node_modules/google-ads-api/build/src/utils.js:76-94](../node_modules/google-ads-api/build/src/utils.js). The walk recurses into nested non-array objects (emitting paths like `value_settings.default_currency_code`) but stops at arrays (emitting the array key as a leaf, e.g. `responsive_search_ad.headlines`). One quirk: the skip filter looks for camelCase `resourceName`, so when we pass snake_case `resource_name` it ends up in the mask too — Google tolerates it silently. Verified by direct probe (2026-05): `getFieldMask({ resource_name, value_settings: { default_currency_code: 'EUR' } })` returns `["resource_name", "value_settings.default_currency_code"]`.
- **Scalar sub-fields are safe to patch.** For `value_settings.*`, `target_spend.cpc_bid_ceiling_micros`, `target_impression_share.*`, and similar scalar sub-messages, sending only the changed sub-field produces a nested mask path and Google updates exactly that field. No need to round-trip siblings — though doing so as a belt-and-suspenders measure (à la `updateAdAssets`'s path1/path2 preservation) is cheap when the GAQL already runs for `beforeValue`.
- **Repeated/array sub-fields wipe siblings.** For RSA `responsive_search_ad.headlines` and `responsive_search_ad.descriptions` the library emits the array key as a leaf in the mask path. **Empirically, Google treats this as "replace the entire `responsive_search_ad` sub-message,"** wiping `path1` / `path2` / pinned-position metadata if they're not re-sent. The root cause hasn't been isolated (server semantics? library version drift?), but the defensive workaround in [campaign-ops.ts:1429-1433](../lib/google-ads/campaign-ops.ts) (read existing sub-message, re-send unchanged fields) is required until proven otherwise. Verify against [all-tools-proto-validation.test.ts](../__tests__/all-tools-proto-validation.test.ts) — the "updateAdAssets — path1/path2 preservation (wipe-bug fix)" describe block pins the read-and-resend pattern. The mocks at line 108-114 and [mutate-e2e-format.test.ts:67-71](../__tests__/mutate-e2e-format.test.ts) are simplified one-level approximations — they DON'T reproduce the library's recursion — so don't use them to predict mask shape for new mutations; run the real `getFieldMask` against your resource shape instead.
- **Empty sub-messages get no mask path.** `{ target_spend: {} }` would produce a mask without any `target_spend.*` entries, so the strategy update silently doesn't land. Always set at least one scalar inside the sub-message — see [writes.ts MAXIMIZE_CLICKS branch](../lib/google-ads/writes.ts) which writes `target_spend.cpc_bid_ceiling_micros` even when the caller didn't pass a ceiling, defaulting to 10B micros = effectively uncapped.
...
- **Field-mask wipe risk on RSA**: see "Library — `google-ads-api` wire format" → repeated sub-fields above. `updateAdAssets` re-sends path1/path2 from existing state for this reason.

## Mutations — silent overwrite traps

- **`updateCampaignBidding` MAXIMIZE_CLICKS used to hardcode `target_spend.cpc_bid_ceiling_micros = 10_000_000_000`** ($10K effectively-uncapped) regardless of the caller's `cpcBidCeilingMicros`. Users hit the trap when they'd switch to Maximize Clicks and pass a 1.0 ceiling — the API returned success and an opaque changeId, but Google's cap stayed at 10K. Pattern to verify when adding bidding-strategy branches: assert (a) the user value lands in `op.resource.<strategy>.<field>` AND (b) the same value is reported in `afterValue` JSON. Fixed by reading `params.cpcBidCeilingMicros ?? 10_000_000_000` and including the effective value in `afterValue`.
...
- `asset.status` — not a field. Use the link status: `customer_asset.status`, `campaign_asset.status`, `ad_group_asset.status`, or `asset_group_asset.status`.
- `asset_group_asset.performance_label` — not a field. Use `asset_group_asset.field_type` / `status` plus metrics from supported resources.
...
- `campaign.budget_micros` — not a field. Budget lives on `campaign_budget.amount_micros`.
- `campaign_criterion.proximity.address.city` — not a field. Use `campaign_criterion.proximity.address.city_name`.
...
- `campaign_experiment.*` — several commonly guessed fields are not available in this Ads API surface. Call `getResourceMetadata('experiment')` / `getResourceMetadata('campaign_experiment')` or use the dedicated experiment tools.
- `conversion_action.default_value` — not a field. Use `conversion_action.value_settings.default_value`.
- `conversion_action.value_settings.currency_code` — not a field. Use `conversion_action.value_settings.default_currency_code`. Google's general API docs sometimes describe `ValueSettings.currency_code`, but the Google Ads v22 proto and the `google-ads-api` library both expose it as `default_currency_code` (parallels `default_value`). Writing `currency_code` may be silently dropped at the library boundary; agents should use the snake_case `default_currency_code` in resource payloads. The MCP's `updateConversionAction` translates `currencyCode` → `default_currency_code` internally.
- `conversion_action.last_conversion_date` / `conversion_action.most_recent_conversion_date` — not fields. Query recent conversion activity from campaign/ad_group metrics segmented by `segments.conversion_action_name`.
