...
export type TelemetryPlatform = 'google_ads' | 'meta_ads';
...
const PLATFORM_TABS: { value: TelemetryPlatform; label: string; href: string }[] = [
    { value: 'google_ads', label: 'Google Ads', href: '/dev/telemetry/google-ads' },
    { value: 'meta_ads', label: 'Meta Ads', href: '/dev/telemetry/meta-ads' },
];
...
const cache = new Map<string, TelemetryPayload>();
const cacheKey = (platform: TelemetryPlatform, days: number, includeDev: boolean) =>
    `${platform}:${days}:${includeDev ? 'dev' : 'prod'}`;

export default function TelemetryDashboard({ platform }: { platform: TelemetryPlatform }) {
    const [days, setDays] = useState(7);
    const [includeDev, setIncludeDev] = useState(false);
    const [data, setData] = useState<TelemetryPayload | null>(cache.get(cacheKey(platform, 7, false)) ?? null);
    ...
    const load = useCallback(async (background = false) => {
        ...
        setError(null);
        try {
            const params = new URLSearchParams({ days: String(days), platform });
            if (includeDev) params.set('includeDev', '1');
            const res = await fetch(`/api/dev/telemetry?${params.toString()}`, { credentials: 'include' });
            if (!res.ok) {
                if (res.status === 403) {
            ...
            setData(payload);
            cache.set(cacheKey(platform, days, includeDev), payload);
        } catch (e) {
            setError(e instanceof Error ? e.message : 'Failed to load telemetry');
        } finally {
            setLoading(false);
        }
    }, [days, platform, includeDev]);

    useEffect(() => {
        const cached = cache.get(cacheKey(platform, days, includeDev));
        if (cached) setData(cached);
        else setData(null);
        load(!!cached);
    }, [load, days, platform, includeDev]);
    ...
    return (
        <div className="min-h-full px-4 py-6 md:px-8">
            <div className="mx-auto max-w-[1400px]">
                ...
                <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            title={
                                includeDev
                                    ? 'Including DEV_EMAILS rows. Click to hide.'
                                    : 'Hiding DEV_EMAILS rows. Click to include them.'
                            }
                            className={`h-8 rounded-md border px-3 text-[12px] font-medium transition ${
                                includeDev
                                    ? 'border-[#D4882A]/40 bg-[#D4882A]/10 text-[#D4882A]'
                                    : 'border-[#3D3C36] bg-[#24231F] text-[#C4C0B6] hover:text-[#E8E4DD]'
                            }`}
                        >
                            {includeDev ? 'Including dev' : 'Excluding dev'}
                        </button>
                ...
                {/* Platform tabs */}
                <div className="mb-3 inline-flex rounded-lg border border-[#3D3C36] bg-[#24231F] p-0.5">
                    {PLATFORM_TABS.map((p) => {
